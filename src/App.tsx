...
style.textContent = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  input,textarea,select {
    ...
    border-radius:10px; padding:10px 14px; outline:none; width:100%; transition:border-color 0.2s;
  }
...
const DISC_COLOR = { swim:"#60c8f0", bike:"#c8f060", run:"#f060a0", rest:"#6a6a8a", strength:"#f0a060" };
const DISC_ICON  = { swim:"🏊", bike:"🚴", run:"🏃", rest:"😴", strength:"💪" };
...
function getWeeks() {
  ...
  while (cur <= end) {
    ...
    const daysToRace = Math.ceil((end - cur) / 86400000);
    let phase = "base";
...
function formatDate(d)   { return new Date(d).toLocaleDateString("nl-BE",{weekday:"short",day:"numeric",month:"short"}); }
function formatShort(d)  { return new Date(d).toLocaleDateString("nl-BE",{day:"numeric",month:"short"}); }
function weekLoad(ts)    { return ts.filter(t=>t.completed&&daysUntil(t.date)>=-7&&daysUntil(t.date)<=0).reduce((s,t)=>s+t.duration,0); }
...
function BottomNav({ tab, setTab }) {
  ...
  return (
    <nav style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:430, background:"rgba(10,10,15,0.97)", backdropFilter:"blur(20px)", borderTop:"1px solid var(--border)", display:"flex", zIndex:100, paddingBottom:"env(safe-area-inset-bottom,0px)" }}>
      {tabs.map(t => (
        <button key={t.id} onClick={()=>setTab(t.id)} style={{ flex:1, padding:"11px 4px 9px", background:"none", display:"flex", flexDirection:"column", alignItems:"center", gap:3, color:tab===t.id?"var(--accent)":"var(--muted)", transition:"color 0.2s" }}>
          ...
          <span style={{ fontSize:9, fontFamily:"var(--font-mono)", letterSpacing:"0.05em" }}>{t.label}</span>
        </button>
      ))}
    </nav>
  );
...
function TodayTab({ trainings, events }) {
  ...
  return (
    <div style={{ padding:"0 16px 110px", display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <Card style={{ background:"linear-gradient(135deg,#1a1a26,#0a1520)", border:"1px solid var(--accent2)33" }}>
        ...
        <div style={{ marginTop:14, display:"flex", gap:4 }}>
          {["BASIS","OPBOUW","PIEK","TAPER","RACE"].map((ph,i) => {
            ...
            const active = i === 1; // currently in build
            return <div key={ph} style={{ flex:1, height:4, borderRadius:2, background: active ? colors[i] : colors[i]+"33" }} />;
          })}
        </div>
      ...
      {todayT && (
        <Card>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <div style={{ width:48, height:48, borderRadius:12, background:DISC_COLOR[todayT.discipline]+"22", border:`1px solid ${DISC_COLOR[todayT.discipline]}44`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:22 }}>
              {DISC_ICON[todayT.discipline]}
            </div>
            <div style={{ flex:1 }}>
              <p style={{ fontWeight:700, fontSize:16 }}>{todayT.type}</p>
              <p style={{ color:"var(--muted)", fontSize:12, fontFamily:"var(--font-mono)" }}>{todayT.duration} min · {formatDate(todayT.date)}</p>
            </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
        {[
          { l:"WEEK MIN",  v:load+"'",           c:"var(--accent)" },
          { l:"SESSIES",   v:done.length,         c:"var(--accent2)" },
          { l:"RvV",       v:daysUntil("2026-04-06")+"d", c:"var(--accent3)" },
        ].map(s=>(
          <Card key={s.l} style={{ textAlign:"center", padding:"14px 8px" }}>
            <p style={{ fontSize:22, fontWeight:800, color:s.c }}>{s.v}</p>
            <p style={{ fontSize:9, fontFamily:"var(--font-mono)", color:"var(--muted)", marginTop:4 }}>{s.l}</p>
          </Card>
        ))}
      </div>
...
function SchemaTab({ trainings, setTrainings }) {
  ...
  function getWeekKey(w) { return w.start; }

  function getSessionsForWeek(week) {
    const key = getWeekKey(week);
    if (weekSessions[key]) return weekSessions[key];
    const tmpl = DEFAULT_WEEK_TEMPLATES[week.phase] || DEFAULT_WEEK_TEMPLATES.base;
    return tmpl.map((s,i) => {
      const d = new Date(week.start); d.setDate(d.getDate()+i);
      return { ...s, id: `${key}-${i}`, date: d.toISOString().slice(0,10), dayIdx: i };
    });
  }

  function saveWeekSessions(week, sessions) {
    setWeekSessions(prev => ({ ...prev, [getWeekKey(week)]: sessions }));
  }

  function applyWeekToLog(week) {
    const sessions = getSessionsForWeek(week);
    const toAdd = sessions.filter(s => s.discipline !== "rest" && s.duration > 0).map(s => ({
      id: Date.now() + Math.random(),
      date: s.date, type: s.type, discipline: s.discipline,
      duration: s.duration, distance: 0, rpe: 0,
      notes: s.focus, completed: false, planned: true,
    }));
    setTrainings(prev => {
  ...
  async function generateAIWeek(week) {
    setGenerating(true);
    const phase = PHASE_COLORS[week.phase];
    const daysLeft = week.daysToRace;
    const prompt = `Je bent triatleetcoach. Genereer een trainingsweek voor Jonas (38j, Ironman Klagenfurt 14 juni, FTP ~300W, lopen HM PB 1:31:59, zwemmen zwakste discipline ~1:36/100m). 
    
Fase: ${phase.name} (${daysLeft} dagen tot race).
Week start: ${week.start}.

Geef 7 sessies (ma t/m zo) als JSON array. Elk object:
{ "day": "Ma", "discipline": "swim|bike|run|rest|strength", "type": "Zwemmen|Fietsen|Lopen|Rust|Kracht", "duration": number_in_minutes, "focus": "korte beschrijving", "keyWorkout": true/false }

Enkel JSON, geen uitleg, geen markdown.`;
    ...
    try {
      ...
      const sessions = parsed.map((s,i) => {
        const d = new Date(week.start); d.setDate(d.getDate()+i);
        return { ...s, id:`${getWeekKey(week)}-ai-${i}`, date:d.toISOString().slice(0,10), dayIdx:i };
      });
      saveWeekSessions(week, sessions);
    ...
    return (
      <div style={{ padding:"56px 16px 110px" }} className="fade-up">
        ...
        <div style={{ display:"flex", gap:8, marginBottom:16 }}>
          ...
          <button onClick={()=>applyWeekToLog(selectedWeek)} style={{ flex:1, padding:"10px", borderRadius:12, fontSize:13, fontWeight:700, background:"var(--surface2)", color:"var(--text)", border:"1px solid var(--border)" }}>
            → Zet in log
          </button>
        </div>
        ...
        <Card style={{ marginBottom:14, padding:"12px 16px" }}>
          <div style={{ display:"flex", justifyContent:"space-between" }}>
            <div style={{ textAlign:"center" }}>
              <p style={{ fontSize:20, fontWeight:800, color:"var(--accent)" }}>{totalMin}'</p>
              <p style={{ fontSize:9, fontFamily:"var(--font-mono)", color:"var(--muted)" }}>TOTAAL</p>
            </div>
            <div style={{ textAlign:"center" }}>
              <p style={{ fontSize:20, fontWeight:800, color:"var(--accent2)" }}>{sessions.filter(s=>s.discipline==="swim").reduce((a,b)=>a+b.duration,0)}'</p>
              <p style={{ fontSize:9, fontFamily:"var(--font-mono)", color:"var(--muted)" }}>ZWEM</p>
            </div>
            <div style={{ textAlign:"center" }}>
              <p style={{ fontSize:20, fontWeight:800, color:"var(--accent)" }}>{sessions.filter(s=>s.discipline==="bike").reduce((a,b)=>a+b.duration,0)}'</p>
              <p style={{ fontSize:9, fontFamily:"var(--font-mono)", color:"var(--muted)" }}>FIETS</p>
            </div>
            <div style={{ textAlign:"center" }}>
              <p style={{ fontSize:20, fontWeight:800, color:"var(--accent3)" }}>{sessions.filter(s=>s.discipline==="run").reduce((a,b)=>a+b.duration,0)}'</p>
              <p style={{ fontSize:9, fontFamily:"var(--font-mono)", color:"var(--muted)" }}>LOOP</p>
            </div>
          </div>
        </Card>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {sessions.map((s,i)=>(
            <Card key={s.id} style={{ display:"flex", alignItems:"center", gap:12, background: s.keyWorkout?"#1a1a26":"var(--surface)", border: s.keyWorkout?`1px solid ${DISC_COLOR[s.discipline]}55`:"1px solid var(--border)" }}>
              <div style={{ width:36, textAlign:"center" }}>
                <p style={{ fontSize:10, fontFamily:"var(--font-mono)", color:"var(--muted)", fontWeight:600 }}>{DAYS[i]}</p>
              </div>
              <div style={{ width:36, height:36, borderRadius:8, background:DISC_COLOR[s.discipline]+"22", border:`1px solid ${DISC_COLOR[s.discipline]}44`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0 }}>
                {DISC_ICON[s.discipline]}
              </div>
    ...
    <div style={{ padding:"56px 16px 110px" }} className="fade-up">
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
        {WEEKS.map((w,i)=>{
          ...
          const totalMin = sessions.reduce((s,t)=>s+t.duration,0);
          const isCurrentWeek = daysUntil(w.start) <= 0 && daysUntil(w.end) >= 0;
          const isPast = daysUntil(w.end) < 0;
          const disc = sessions.map(s=>s.discipline);

          return (
            <div key={w.start} onClick={()=>setSelectedWeek(w)} style={{
              background: isCurrentWeek ? phase.bg : "var(--surface)",
              border: isCurrentWeek ? phase.border : "1px solid var(--border)",
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <div>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4 }}>
                    {isCurrentWeek && <div style={{ width:6, height:6, borderRadius:"50%", background:phase.label, animation:"pulse 2s infinite" }} />}
                    <span style={{ fontSize:10, fontFamily:"var(--font-mono)", color:phase.label, fontWeight:600 }}>{phase.name}</span>
                    {isCurrentWeek && <span style={{ fontSize:10, fontFamily:"var(--font-mono)", color:"var(--muted)" }}>← NU</span>}
                  </div>
              ...
              {/* Mini discipline strip */}
              <div style={{ display:"flex", gap:3, marginTop:10 }}>
                {disc.map((d,j)=>(
                  <div key={j} style={{ flex:1, height:4, borderRadius:2, background: DISC_COLOR[d]+"66" }} />
                ))}
              </div>
            </div>
          );
  ...
  return (
    <div style={{ padding:"56px 16px 110px" }} className="fade-up">
      ...
      <Card style={{ marginBottom:14 }}>
        <p style={{ fontSize:11, fontFamily:"var(--font-mono)", color:"var(--muted)", marginBottom:12 }}>WEEK BELASTING · {load} min</p>
          ...
          {["2026-03-25","2026-03-26","2026-03-27","2026-03-28","2026-03-29","2026-03-30","2026-03-31"].map((date,i)=>{
            ...
            return (
              <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
                ...
                <span style={{ fontSize:9, fontFamily:"var(--font-mono)", color:isToday?"var(--accent)":"var(--muted)" }}>
      ...
      {showAdd && (
        <Card style={{ marginBottom:14 }} className="fade-up">
          ...
          {[
            ...
            { label:"Type",        field:"type",       type:"select", opts:["Zwemmen","Fietsen","Lopen","Rust","Kracht"] },
            { label:"Discipline",  field:"discipline", type:"select", opts:["swim","bike","run","rest","strength"] },
            { label:"Duur (min)",  field:"duration",   type:"number", ph:"90" },
            { label:"Afstand",     field:"distance",   type:"number", ph:"0" },
            { label:"RPE (1–10)",  field:"rpe",        type:"number", ph:"6" },
            { label:"Notities",    field:"notes",      type:"textarea" },
          ].map(f=>(
            <div key={f.field} style={{ marginBottom:10 }}>
              <p style={{ fontSize:11, fontFamily:"var(--font-mono)", color:"var(--muted)", marginBottom:5 }}>{f.label.toUpperCase()}</p>
              {f.type==="select" ? (
                <select value={form[f.field]} onChange={e=>setForm(p=>({...p,[f.field]:e.target.value}))}>
                  {f.opts.map(o=><option key={o}>{o}</option>)}
                </select>
              ) : f.type==="textarea" ? (
                <textarea value={form[f.field]} onChange={e=>setForm(p=>({...p,[f.field]:e.target.value}))} placeholder={f.ph}/>
              ) : (
                <input type={f.type} value={form[f.field]} onChange={e=>setForm(p=>({...p,[f.field]:e.target.value}))} placeholder={f.ph}/>
              )}
            </div>
          ))}
          <label style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8, background:"var(--surface2)", border:"1px dashed var(--border)", borderRadius:10, padding:14, cursor:"pointer", fontSize:13, color:"var(--muted)", marginBottom:10 }}>
            <input type="file" accept=".gpx,.fit" style={{ display:"none" }} onChange={e=>setGpxFile(e.target.files[0])}/>
            {gpxFile ? `✓ ${gpxFile.name}` : "📎 GPX / FIT uploaden"}
          </label>
          ...
          <Card key={t.id} onClick={()=>setSelected(t)} style={{ display:"flex", alignItems:"center", gap:12 }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <p style={{ fontSize:11, color:"var(--muted)", fontFamily:"var(--font-mono)" }}>
                {formatDate(t.date)} · {t.duration}min{t.distance?` · ${t.distance}${t.discipline==="swim"?"m":"km"}`:""}
              </p>
              {t.notes && <p style={{ fontSize:11, color:"var(--muted)", marginTop:1, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{t.notes}</p>}
            </div>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", backdropFilter:"blur(8px)", display:"flex", alignItems:"flex-end", zIndex:200 }} onClick={()=>setSelected(null)}>
          <div onClick={e=>e.stopPropagation()} style={{ background:"var(--surface)", borderRadius:"20px 20px 0 0", border:"1px solid var(--border)", padding:24, width:"100%" }} className="fade-up">
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:16 }}>
              {[
                { l:"Duur",     v:selected.duration+" min" },
                { l:"Afstand",  v:selected.distance?`${selected.distance}${selected.discipline==="swim"?"m":"km"}`:"—" },
                { l:"RPE",      v:selected.rpe||"—" },
              ].map(s=>(
                <div key={s.l} style={{ background:"var(--surface2)", borderRadius:10, padding:12, textAlign:"center" }}>
                  <p style={{ fontSize:16, fontWeight:700, color:"var(--accent)" }}>{s.v}</p>
                  <p style={{ fontSize:10, fontFamily:"var(--font-mono)", color:"var(--muted)", marginTop:2 }}>{s.l}</p>
                </div>
              ))}
            </div>
    ...
    <div style={{ padding:"56px 16px 110px" }} className="fade-up">
      ...
      {showAdd && (
        <Card style={{ marginBottom:14 }} className="fade-up">
          {[
            { label:"Datum", field:"date", type:"date" },
            { label:"Titel", field:"title", type:"text", ph:"bv. Doktersafspraak" },
          ].map(f=>(
            <div key={f.field} style={{ marginBottom:10 }}>
              <p style={{ fontSize:11, fontFamily:"var(--font-mono)", color:"var(--muted)", marginBottom:5 }}>{f.label.toUpperCase()}</p>
              <input type={f.type} value={form[f.field]} onChange={e=>setForm(p=>({...p,[f.field]:e.target.value}))} placeholder={f.ph}/>
        ...
        {sorted.map(e=>{
          ...
          return (
            <Card key={e.id} style={{ opacity:isPast?0.5:1, display:"flex", alignItems:"center", gap:12 }}>
              ...
              <Badge color={isPast?"var(--muted)":e.type==="race"?"var(--accent3)":"var(--accent2)"}>
                {isPast?"VOORBIJ":days===0?"VANDAAG":`${days}d`}
              </Badge>
            </Card>
          );
        })}
...
function BriefTab({ trainings, events }) {
  ...
  async function generate() {
    ...
    const upcoming = trainings.filter(t=>!t.completed&&t.planned).slice(0,5);
    const nextEvents = events.filter(e=>daysUntil(e.date)>=0).slice(0,3);
    const prompt = `Je bent de personal coach van Jonas, 38j triatleet, Ironman Klagenfurt 14 juni (75 dagen). FTP ~300W (doel 350W), gewicht ~79kg (doel 74-75kg), HM PB 1:31:59, zwemmen zwakste ~1:36/100m. Nieuwe job als F&B Manager voco Gent start 1 april.
...
Gepland:
${upcoming.map(t=>`- ${t.date}: ${t.type} ${t.duration}min`).join('\n')}

Events: ${nextEvents.map(e=>`${e.title} (${daysUntil(e.date)}d)`).join(', ')}

Focus: ${focus}

Schrijf een directe, persoonlijke coach-briefing in NEDERLANDS. Gebruik deze structuur:
**Status** – hoe staat Jonas ervoor op basis van de data (2-3 zinnen)
**Vandaag** – concreet advies training + timing
**Voeding & herstel** – 2-3 specifieke adviezen
**Opgelet** – 1-2 risico's of aandachtspunten
**Drive** – 1 motiverende zin specifiek voor Jonas
...
Wees direct en specifiek, geen algemeenheden.`;
    ...
    } catch(e) { setBrief("Verbinding mislukt."); }
  ...
  function renderBrief(text) {
    return text.split('\n').map((line,i)=>{
      ...
      const isHeader = /^\*\*/.test(line) || /^\d\./.test(line);
      return <p key={i} style={{ fontSize: isHeader?15:14, fontWeight:isHeader?700:400, lineHeight:1.65, marginTop:isHeader?14:0, color:"var(--text)", opacity: isHeader?1:0.88 }} dangerouslySetInnerHTML={{ __html:html }}/>;
    });
  }
  ...
  return (
    <div style={{ padding:"56px 16px 110px" }} className="fade-up">
      ...
      <button onClick={generate} disabled={loading} style={{ width:"100%", padding:"16px", borderRadius:14, fontSize:16, fontWeight:700, background:loading?"var(--surface2)":"linear-gradient(135deg,var(--accent),var(--accent2))", color:loading?"var(--muted)":"#0a0a0f", border:"none", marginBottom:18, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
      ...
      {brief && (
        <Card style={{ background:"linear-gradient(180deg,#12121a,#0d0d18)" }} className="fade-up">
          ...
          <div style={{ borderTop:"1px solid var(--border)", paddingTop:14 }}>{renderBrief(brief)}</div>
        </Card>
      )}
      ...
      {!brief && !loading && (
        <Card style={{ textAlign:"center", padding:32 }}>
          ...
          <p style={{ color:"var(--muted)", fontSize:13, lineHeight:1.6 }}>Kies een focus en druk op genereren voor je dagelijkse briefing op basis van jouw trainingsdata.</p>
        </Card>
      )}
    </div>
  );
